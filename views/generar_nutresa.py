...
generar_nutresa_bp = Blueprint(
    "generar_nutresa", __name__,
...
@generar_nutresa_bp.route("/generar-nutresa", methods=["GET", "POST"])
@login_required
def generar_nutresa_index():
    ...
    if request.method == "POST":
        try:
            ...
            if not f_inv:
                ...
                return redirect(url_for("generar_nutresa.generar_nutresa_index"))
            ...
            flash("Pedidos Nutresa generados con éxito.", "success")
            return redirect(url_for("generar_nutresa.generar_nutresa_index", descarga=1))

        except Exception as e:
            flash(f"Error al generar pedidos Nutresa: {e}", "error")
            return redirect(url_for("generar_nutresa.generar_nutresa_index"))

    return render_template(
        "generar_nutresa.html",
        mostrar_descarga=mostrar_descarga
    )

@generar_nutresa_bp.route("/generar-nutresa/descargar", methods=["GET"])
@login_required
def descargar_reportes_nutresa():
    empresa = session.get('empresa')
    conn = conectar(); cur = conn.cursor()
    ...
    with zipfile.ZipFile(zip_buf, "w") as zf:
        b_rep = BytesIO()
        pd.DataFrame(data_rep).to_excel(b_rep, sheet_name="Reparticion", index=False, engine="openpyxl")
        b_rep.seek(0)
        ...
        for ruta in rutas:
            ...
            with pd.ExcelWriter(buf, engine="openpyxl") as writer:
                df.to_excel(writer, sheet_name=f"Ruta_{ruta}", index=False, startrow=3)
            buf.seek(0)
            zf.writestr(f"pedidos_ruta_{ruta}.xlsx", buf.read())

    zip_buf.seek(0)
    hoy = datetime.now().strftime("%Y%m%d")
    nombre_zip = f"reportes_nutresa_{hoy}.zip"
    return send_file(
        zip_buf,
        as_attachment=True,
