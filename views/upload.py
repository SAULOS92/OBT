...
def normalize_cols(df: pd.DataFrame, col_map: dict) -> pd.DataFrame:
    """
    Renombra columnas según col_map (sin limpieza extra de nombres).
    """
    inv = {}
...
def upload_index():
    if request.method == "POST":
        ...
        if not f_ped or not f_rut or p_dia not in DIAS_VALIDOS:
            flash("Sube ambos Excel y selecciona un día válido.", "error")
        ...
        # 3) Normalizar nombres via COL_MAP
        ...
        # 6) Rellenar nombre/barrio/ciudad faltantes por cliente
        ...
        # 7) Forzar numéricos y rellenar ceros
        try:
            df_ped["codigo_pro"] = df_ped["codigo_pro"].replace("", "0").astype(int)
        ...
        # 10) Generar y devolver Excel de resumen
        try:
            conn = conectar(); cur = conn.cursor()
            ...
            flash(f"Error obteniendo resumen: {e}", "error")
            return redirect(request.url)
