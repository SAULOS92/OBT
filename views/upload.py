...
def upload_index():
    if request.method == "POST":
        ...
        # 6.2) Texto en número y cliente
        if "numero_pedido" in df_ped.columns:
            df_ped["numero_pedido"] = df_ped["numero_pedido"].map(lambda v: None if v is None else str(v))
        if "cliente" in df_ped.columns:
            df_ped["cliente"] = df_ped["cliente"].map(lambda v: None if v is None else str(v))
        if "codigo_cliente" in df_rut.columns:
            df_rut["codigo_cliente"] = df_rut["codigo_cliente"].map(lambda v: None if v is None else str(v))
        # Validar encabezados
