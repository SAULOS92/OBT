    ...
    if request.method == "POST":
        try:
            ...
            if negocio != "nutresa":
                ...
                df_mat["particion"] = df_mat["particion"].str.strip().replace("", "1").astype(float).astype(int)
                df_mat["pq_x_caja"] = df_mat["pq_x_caja"].str.strip().replace("", "1").astype(float).astype(int)
